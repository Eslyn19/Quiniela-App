...
export const obtenerPuntosDisponibles = async (req, res) => {
    ...
    try {
        const [row] = await sequelize.query(
            `EXEC ObtenerPuntosUsuario :id_usuario`,
            { 
        ...
        return res.status(500).json({ 
            message: `Error al obtener puntos, ${error.message}` 
        });
    }
};
    ...
    try {
        ...
        if (premio.stock !== null && premio.stock <= 0) {
            ...
            return res.status(400).json({ 
                message: 'Premio sin stock disponible' 
            });
        }
        ...
        if (premio.fecha_inicio && new Date(premio.fecha_inicio) > hoy) {
            await t.rollback();
            return res.status(400).json({ 
                message: 'El premio aún no está disponible' 
            });
        }
        if (premio.fecha_fin && new Date(premio.fecha_fin) < hoy) {
            await t.rollback();
            return res.status(400).json({ 
                message: 'El premio ha expirado' 
            });
        }
        ...
        if (disponible < premio.costo_puntos) {
            await t.rollback();
            return res.status(400).json({
                message: `Puntos insuficientes. Tienes ${disponible} puntos disponibles`,
            });
        }
        ...
        return res.status(201).json({ 
            message: 'Canje realizado exitosamente', canje 
        });
    } catch (error) {
        await t.rollback();
        return res.status(500).json({ 
            message: `Error al canjear premio, ${error.message}`
        });
    }
};
...
export const obtenerMisCanjes = async (req, res) => {
    const id_usuario = req.user.id_usuario;
    try {
        const canjes = await sequelize.query(
            `EXEC ObtenerMisCanjes :id_usuario`,
            { 
                replacements: { id_usuario }, 
                type: sequelize.QueryTypes.SELECT 
            }
