...
export const obtenerPremios = async (req, res) => {
    try {
        ...
        return res.status(500).json({ 
            message: `Error al obtener premios, ${error.message}` 
        });
    }
};

export const crearPremio = async (req, res) => {
    const { nombre, descripcion, costo_puntos, stock, fecha_inicio, fecha_fin, imagen_url } = req.body;

    if (!nombre?.trim() || !descripcion?.trim() || !costo_puntos) {
        return res.status(400).json({ message: 'Nombre, descripción y costo son requeridos' });
    }
    ...
    try {
        ...
        return res.status(500).json({ 
            message: `Error al crear premio, ${error.message}` 
        });
    }
};
...
export const actualizarPremio = async (req, res) => {
    try {
        ...
        const { nombre, descripcion, costo_puntos, 
                stock, activo, fecha_inicio, fecha_fin, 
                imagen_url } = req.body;

        await premio.update({
            nombre: nombre?.trim() ?? premio.nombre,
            descripcion: descripcion?.trim() ?? premio.descripcion,
            costo_puntos: costo_puntos !== undefined ? Number(costo_puntos) : premio.costo_puntos,
            stock: stock !== undefined ? (stock === '' ? null : Number(stock)) : premio.stock,
            activo: activo !== undefined ? activo : premio.activo,
            fecha_inicio: fecha_inicio !== undefined ? (fecha_inicio || null) : premio.fecha_inicio,
            fecha_fin: fecha_fin !== undefined ? (fecha_fin    || null) : premio.fecha_fin,
            imagen_url: imagen_url !== undefined ? (imagen_url   || null) : premio.imagen_url,
        });
...
export const actualizarEstadoCanje = async (req, res) => {
    ...
    try {
        ...
        if (!canje) return res.status(404).json({ 
