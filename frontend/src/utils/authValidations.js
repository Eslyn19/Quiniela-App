...
const validateLetterField = (value, fieldName) => {
    if (!onlyLettersRegex.test(value.trim())) return `${fieldName} solo permite letras.`;
    return '';
};
...
export const validateRegisterForm = (form) => {
    ...
    if (form.username.trim().length < 3) return 'El nombre de usuario debe tener al menos 3 caracteres.';
    if (/\s/.test(form.username)) return 'El nombre de usuario no puede contener espacios.';
