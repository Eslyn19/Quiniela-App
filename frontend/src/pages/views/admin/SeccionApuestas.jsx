...
export default function SeccionApuestas() {
    ...
    const handleSubmit = async e => {
        ...
        if (dtFin <= dtInicio) {
            ...
            setError(mismodia ? `Mismo día: la hora de fin (${fmt(dtFin)}) debe ser mayor a la hora de inicio (${fmt(dtInicio)}).` : 'La fecha de fin debe ser posterior a la fecha de inicio.');
            return;
        }

        setSubmitting(true);
        try {
            const res = await fetch(`${API}/api/admin/apuestas`, 
            { 
                method: 'POST', 
                headers: getHeaders(), 
                body: JSON.stringify({
            ...
            if (!res.ok) { 
                setError(data.message || data.errors?.join(', ') || 'Error al crear apuesta.'); 
                return;
            }
                ...
                <form className="admin-form" onSubmit={handleSubmit} autoComplete='off'>
                    ...
                    <div className="apuesta-form-grid">
                        ...
                        <div className="form-group">
                            ...
                            <select 
                                name="id_tipo_puntuacion" 
                                value={form.id_tipo_puntuacion} 
                                onChange={handleChange}
                            >
                                <option value="">Seleccionar...</option>
                                {puntuaciones.map(p => (
                                    <option key={p.id_tipo_puntuacion} value={p.id_tipo_puntuacion}>
                                        {p.nombre}
                                    </option>
                                ))}
                            </select>
                        ...
                        <div className="form-group">
                            <label>Estado</label>
                            <select 
                                name="id_estado_apuesta" 
                                value={form.id_estado_apuesta} 
                                onChange={handleChange}
                            >
                                <option value="">Seleccionar...</option>
                                {ESTADOS_APUESTA.map(e => (
                                    <option key={e.id_estado_apuesta} value={e.id_estado_apuesta}>
                                        {e.nombre}
                                    </option>
                                ))}
                            </select>
                        ...
                        <tbody>
                            {apuestas.map((a, i) => (
                                <tr key={a.id_apuesta}>
                                    ...
                                    <td className="td-muted">{a.equipo}</td>
                            ...
                            {apuestas.length === 0 && <EmptyRow cols={9} msg="Sin apuestas creadas" />}
                        </tbody>
                    </table>
                </div>
            </div>
